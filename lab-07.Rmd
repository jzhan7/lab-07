...
evals %>% 
  group_by(score) %>% 
  summarise(mean(bty_avg))
  
```

Right skewed means most students rated courses highly. I expected no corellation and this is proven by how the lowest score has an average beauty rating of greater than 5.
...
geom_jitter adds random variation to the data points for a more detailed plot compared to geom_point 
...
score_bty_fit_aug %>% ggplot(aes(x = bty_avg, y = score))+
  geom_jitter()+
  geom_smooth(method="lm")+
  labs(x = "Beauty", y = "Score")
```
...
R^2 = 0.03502226, R^2 far from 1 so no correlation.
